...
static void *do_gups(void *arguments)
{
  ...
  // set_mempolicy(MPOL_BIND, &nodemask, 4);
  __parsec_roi_begin();
...
int main(int argc, char *argv[])
{
  ...
  if (argc != 10)
  {
    ...
    printf("  mempolicy\t\t\t memory policy: interleave, default, mbind, preferred, local");
    return 0;
  }
  ...
  // set mempolicy()
    // MPOL_BIND: the system will kill the task if all the node have noe free memory.
  /**
   * With only cpu-less memory, it will kill the taks if one of the cpu-less node is full.
  */
  int mempolicy = atoi(argv[9]);

  unsigned long nodemask = 15UL ;
  printf("\n nodemask = : %d\n", nodemask);
  switch (mempolicy)
  {
  case 0: // default
  printf("mempolicy : default\n");
    set_mempolicy(MPOL_DEFAULT, NULL, 8);
    break;
  case 1:
    printf("mempolicy : interleave\n");

    set_mempolicy(MPOL_INTERLEAVE, &nodemask, 8);
    break;
  case 2:
      printf("mempolicy : bind\n");

    set_mempolicy(MPOL_BIND, &nodemask, 8);
    break;
  case 3:
        printf("mempolicy : preferred\n");

    set_mempolicy(MPOL_PREFERRED, &nodemask, 8);
    break;
  default:
    break;
  };
